        ...
        do {
          ...
          if (star) {
            if (star.Mag > 1 && mode === 'minimal') {
              ...
              radius = luminance / (Math.pow(luminance, 4 / 5) + thresholdLuminance);
          ...
          //Y.log([self.nodeCount, time, start, duration]);
        } while ((time - start < duration) && (self.nodeCount < Y.ACMACS.stars.length)); // for each star

        Y.one('#count_display').setContent(self.nodeCount + ' nodes');

//        Y.later(1, self, function () {
        if (self.nodeCount < Y.ACMACS.stars.length) {
          Y.one('#count_display').simulate('change');
        }
//        });
      }; // loopFor()

      Y.one('#count_display').on('change', function (e) {
        if (self.nodeCount <= Y.ACMACS.stars.length) {
          self.loopFor(100);
